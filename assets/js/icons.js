/**
 * Custom Icon System - Replaces Lucide (382KB) with only the icons used (~15KB)
 * Based on Lucide v0.555.0 - ISC License
 * 
 * Usage: Same as Lucide - use <i data-lucide="icon-name"></i> in HTML,
 * then call renderIcons() to replace them with inline SVGs.
 */
(function() {
    'use strict';
    ...
    var ICONS = {
        ...
        'monitor-dot': '<circle cx="19" cy="6" r="3"/><path d="M22 12v3a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h9"/><path d="M12 17v4"/><path d="M8 21h8"/>',
    ...
    /**
     * Create an SVG string for a given icon name
     * @param {string} name - Icon name (e.g., 'search', 'arrow-left')
     * @param {string} [extraClass] - Additional CSS class(es)
     * @returns {string} SVG markup string, or empty string if icon not found
     */
    window.createIcon = function(name, extraClass) {
     ...
     * Compatible with the Lucide createIcons() API pattern.
     * @param {Element} [root] - Root element to search within (defaults to document)
     */
    window.renderIcons = function(root) {
        var container = root || document;
        var elements = container.querySelectorAll('i[data-lucide]');
        for (var i = 0; i < elements.length; i++) {
            ...
            // Build class list: lucide + lucide-{name} + original classes
            var cls = 'lucide lucide-' + name;
            if (el.className) cls += ' ' + el.className;
            svg.setAttribute('class', cls);

            // Copy data attributes and inline styles
            svg.setAttribute('data-lucide', name);
            if (el.style.cssText) svg.style.cssText = el.style.cssText;

            // Parse and set inner SVG content
            svg.innerHTML = paths;
